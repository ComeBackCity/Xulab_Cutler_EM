...
TOMO_CATEGORIES = [
    {"color": [220], "isthing": 1, "id": 1, "name": "fg"},
]
...
def _get_tomo_instances_meta():
    thing_ids = [k["id"] for k in TOMO_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in TOMO_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 1, len(thing_ids)
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in TOMO_CATEGORIES if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
        "thing_colors": thing_colors,
        "class_image_count":  [{'id': 1, 'image_count': 301}]
    }
    return ret
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name in ["imagenet", "kitti", "cross_domain", "lvis", "voc", "coco_cls_agnostic", "objects365", 'openimages', 'tomo_mito']:
        return _get_imagenet_instances_meta()
    elif dataset_name == "tomo":
        return _get_tomo_instances_meta()
