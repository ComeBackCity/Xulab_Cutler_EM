...
ToTensor_sc = transforms.Compose([transforms.ToTensor(),
                               transforms.Normalize(
                                (0.485),
                                (0.229)),])
...
if __name__ == "__main__":
    ...
    backbone = dino.ViTFeat(url, feat_dim, args.vit_arch, args.vit_feat, args.patch_size, url=False)
    ...
    Img_rgb = Image.open(args.img_path).convert("RGB")
    input = np.array(Img_rgb)
    # input = np.array(I)
