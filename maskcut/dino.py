...
def vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, in_chans=1, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
...
def vit_base(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, in_chans=1, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

class ViTFeat(nn.Module):
    """ Vision Transformer """
    def __init__(self, pretrained_pth, feat_dim, vit_arch = 'base', vit_feat = 'k', patch_size=16, url=True):
        super().__init__()
        if vit_arch == 'base' :
